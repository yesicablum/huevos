...
const Inventory = () => {
    ...
    useEffect(() => {
        axios.get('https://app-huevos-backend.onrender.com/consultar_inventario')  // Ajusta la URL si es necesario
            .then(response => {
        ...
        <div className="inventory-container">
            ...
            <div className="egg-groups">
                <div className="group red-eggs">
                    <h2>Huevos Rojos</h2>
                    <span className="group-total">Total: {inventory.red.reduce((acc, item) => acc + item.cantidad, 0)}</span>
                    <div className="egg-list">
                        {inventory.red.map((item, idx) => (
                            <div key={idx} className="egg-card">
                                <span className="egg-label">Tamaño:</span>
                                <span className="egg-size">{item.tamaño}</span>
                                <span className="egg-label">Cantidad:</span>
                                <span className="egg-count">{item.cantidad}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="group white-eggs">
                    ...
                    <span className="group-total">Total: {inventory.white.reduce((acc, item) => acc + item.cantidad, 0)}</span>
                    <div className="egg-list">
